...
**aha-workspace**는 클로드 코드를 처음 써보는 사람을 위한 작업 환경입니다. 번호 체계로 폴더 위치를 한눈에 알 수 있고, 슬래시 커맨드 몇 개로 기록·정리·할일 관리가 자동으로 이어지도록 만들어졌습니다.
...
| `/gather` | 정보 수집 모드 (구조화된 질문 생성, 답변하면 `/reframe`까지 자동 연결) |
| `/reframe` | 이해 확인 모드 (대화 요약 및 확인). `/gather` 뒤엔 자동 실행, 단독 호출도 가능 |
...
┌─────────────────────────────────────────────────────────────┐
│                    DAILY WORKFLOW                            │
│  /오늘 → 71-daily에 자유 기록 (할일 포함)                    │
│       → 기록에서 할일 자동 추출·정리 → 73-todos              │
└─────────────────────────────────────────────────────────────┘
                              │
                              v
┌─────────────────────────────────────────────────────────────┐
│                ORCHESTRATION COMMANDS                        │
│  /gather → 정보 수집 (구조화된 질문) → 답변 시 자동 이어짐   │
│  /reframe → 이해 확인 (대화 요약, 자동 or 단독 호출)         │
